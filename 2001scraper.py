...
BASE_2001 = "https://www.nrc.gov/reading-rm/doc-collections/event-status/reactor-status/2001"
...
def iter_dates_2001() -> Iterable[date]:
    d = date(2001, 1, 1)
    end = date(2001, 12, 31)
    while d <= end:
        yield d
...
def url_for_day(d: date) -> str:
    return f"{BASE_2001}/{d.strftime('%Y%m%d')}ps.html"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Scrape NRC 2001 daily power reactor status reports.")
    ap.add_argument(
        "--out",
        default=str(Path("output") / "nrc_reactor_status_2001.psv"),
        help="Output pipe-delimited file path.",
    )
    ap.add_argument(
        "--cache-dir",
        default=str(Path(".cache") / "nrc_2001_html"),
    ...
    with out_path.open("w", encoding="utf-8", newline="\n") as f:
        for i, d in enumerate(iter_dates_2001(), start=1):
            url = url_for_day(d)
