...
BASE_2002 = "https://www.nrc.gov/reading-rm/doc-collections/event-status/reactor-status/2002"
...
def iter_dates_2002() -> Iterable[date]:
    d = date(2002, 1, 1)
    end = date(2002, 12, 31)
    while d <= end:
        yield d
...
def url_for_day(d: date) -> str:
    return f"{BASE_2002}/{d.strftime('%Y%m%d')}ps.html"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Scrape NRC 2002 daily power reactor status reports.")
    ap.add_argument(
        "--out",
        default=str(Path("output") / "nrc_reactor_status_2002.psv"),
        help="Output pipe-delimited file path.",
    )
    ap.add_argument(
        "--cache-dir",
        default=str(Path(".cache") / "nrc_2002_html"),
    ...
    with out_path.open("w", encoding="utf-8", newline="\n") as f:
        for i, d in enumerate(iter_dates_2002(), start=1):
            url = url_for_day(d)
